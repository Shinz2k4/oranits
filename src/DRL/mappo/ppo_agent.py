...
class Memory:
    ...
    def __init__(self, global_state = False):
...
class ExperienceDataset(Dataset):
    ...
    def __init__(self, old_states, old_actions, old_log_probs, rewards, old_global_states = []):
        ...
        self.has_global_state = False
        if len(old_global_states)!=0:
            self.old_global_states = old_global_states
            self.has_global_state = True
    ...
    def __getitem__(self, ix):
        ...
        if self.has_global_state:
            return (self.old_states[ix], self.old_actions[ix],
                self.old_log_probs[ix], self.rewards[ix], self.old_global_states[ix])
        else:
            return (self.old_states[ix], self.old_actions[ix],
...
class PPOAgent:
    ...
    def __init__(self, device, actor_critic, actor_critic_old, gamma,
                 ...
                 batch_size, actor_optimizer, critic_optimizer, verbose = True):
        ...
        self.aid = PPOAgent.id
        PPOAgent.id += 1
        self.verbose = verbose


    def add_memory(self, state, action, log_prob, reward, done, global_state = []):
        ...
        if len(global_state)!=0:
            self.memory.create_global_state()
    ...
    def get_actions(self, state):
        ...
        value_pre = value_pre.cpu().detach()
        
        return actions, value_pre
    ...
    def update_batch(self, old_states, old_actions, old_log_probs, rewards):
        ...
        # if (ratios > 20).any() or (ratios < -20).any():
        #     ratios = torch.exp(log_probs - 20)
        #     print("Larger ratios .... happening", ratios)
        ...
        if self.verbose:
            print("Agent ::{} training is in process".format(self.aid))
    ...
    def update(self):
        ...
        with self.lock:
            ...
            old_global_state = []
            if self.memory.has_global_state:
                old_global_state = torch.stack(self.memory.global_state).to(self.device)

            # Initialize ExperienceDataset and DataLoader objects for training.
            dataset = ExperienceDataset(old_states, old_actions,
                                        old_log_probs, rewards, old_global_state)
        ...
        print("exit thread! finish updating policy")