...
def run():
    ...
    a2c = A2C(env=env, memory_capacity=MEMORY_CAPACITY,
              ...
              infor_name = output_dir + "/a2c_infor.txt")

    episodes =[]
    eval_rewards =[]
    
    a2c.log("time to log {}".format(current_datetime_hour))
    while a2c.n_episodes < MAX_EPISODES:
        ...
        if a2c.episode_done and ((a2c.n_episodes+1)%EVAL_INTERVAL == 0):
            rewards, _ = a2c.evaluation(env, EVAL_EPISODES) #env_eval
            ...
            df.to_csv(f"{output_dir}/real_map_a2c_eval_data.csv", index=False)

            plt.figure()
            plt.plot(episodes, eval_rewards, label="A2C")
            plt.title("Real_map")
            plt.xlabel("Episode")
            plt.ylabel("Average Reward")
            plt.legend()
            plt.savefig(f"{output_dir}/A2C_real_map_{a2c.n_episodes + 1}_episodes.png")
            plt.close()
            # a2c.save_model(f"{checkpoint_dir}/check_point_{a2c.n_episodes + 1}.pth")
            a2c.plot_save_loss(output_dir)
            plt.close()
