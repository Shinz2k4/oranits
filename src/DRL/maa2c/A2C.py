...
from DRL.common.utils import entropy, index_to_one_hot, to_tensor_var
...
class A2C(Agent):
    ...
    def __init__(self, env, state_dim, action_dim,
                 ...
                 actor_hidden_size=32, critic_hidden_size=64,
                 actor_output_act=nn.functional.softmax, critic_loss="mse",
                 actor_lr=0.001, critic_lr=0.0001,
                 optimizer_type="rmsprop", entropy_reg=0.01,                    #optimizer_type="rmsprop"
                 max_grad_norm=0.5, batch_size=64, episodes_before_train=100,
                 epsilon_start=0.9, epsilon_end=0.01, epsilon_decay=100,
                 use_cuda=True,
    ...
    def train(self):
        ...
        if not batch:
            return 
        if not self.reduce_ep_start:
            self.log("Start training at n_steps {}".format(self.n_steps))
            self.reduce_ep_start = self.n_steps
        ...
        self.df_loss['actor'].append(actor_loss.item())
        ...
        self.df_loss['critic'].append(critic_loss.item())
    ...
    def convert_action(self,sm_actions, num):
        if not isinstance(sm_actions, (list, np.ndarray)):
            sm_actions = [sm_actions]
        min_val = np.min(sm_actions)
        max_val = np.max(sm_actions)
        if min_val == max_val:
            raise ValueError("All actions have the same value {}".format(sm_actions))
        milestones = (max_val - min_val) / num
        quatize = []
        for i in range(num):
            quatize.append((min_val + i * milestones, min_val + (i + 1) * milestones))
        actions = []
        quatize = list(set(quatize))
        for sma in sm_actions:
            for idx, q in enumerate(quatize):
                if q[0] <= sma <= q[1]:
                    actions.append(idx)
        return actions
    ...
    def exploration_action(self, state):
        ...
        # epsilon = self.epsilon_end + (self.epsilon_start - self.epsilon_end) * np.exp(-1. * self.n_steps / self.epsilon_decay)
